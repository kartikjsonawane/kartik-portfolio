...
model.compile(optimizer=Adam(1e-5), ...)  # 100x smaller lr
\`\`\`
...
This post covers the full pipeline: messy data to production API.
...
Initial model: 500 trees, 40 features, ~45ms per request.

Drop near-zero importance features (40 to 24):
...
Use early stopping's best iteration:

\`\`\`python
model_prod = XGBClassifier(n_estimators=model.best_iteration, ...)
\`\`\`

Result: **29ms** inference, 0.88 ROC-AUC.
